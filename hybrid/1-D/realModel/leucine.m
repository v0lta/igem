classdef leucine<handle
	...
	methods
		function obj=leucine(domain,concentration,boundaries)
		obj.domain=domain;
		...
		function update(obj,bacteriaDensity,Dleucine,beta,k2,dt)
		%Update concentration based on bacteria density, diffusion constant of leucine,
		%production rate (beta) , degradation rate (k2) and timestep
		n=length(obj.domain);
		...
		%A=diag(ones(n,1)*(1+2*Dleucine*r));
		%A=A+diag([-2*Dleucine*r;ones(n-2,1)*(-Dleucine*r)],1);
		%A=A+diag([ones(n-2,1)*(-Dleucine*r);-2*Dleucine*r],-1);
		%b=obj.concentration;

		%fixed concentration boundary condition
		%A=diag(ones(n-2,1)*(1+2*Dleucine*r));
		%A=A+diag(ones(n-3,1)*(-Dleucine*r),1);
		%A=A+diag(ones(n-3,1)*(-Dleucine*r),-1);
		%b=obj.concentration(2:end-1);
		%b(1)=b(1)+Dleucine*r*obj.leftBoundary;
		%b(end)=b(end)+Dleucine*r*obj.rightBoundary;

		%periodic boundary condition
		A=diag(ones(n,1)*(1+2*Dleucine*r+k2*dt))...
		+diag(ones(n-1,1)*(-Dleucine*r),1)...
		+diag(ones(n-1,1)*(-Dleucine*r),-1);
		A(end,1)=-Dleucine*r;
		A(1,end)=-Dleucine*r;
		b=obj.concentration;
