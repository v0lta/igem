...
for t = 1:1:steps
    ...
    B(t+1,idx) = B(t,idx) +  ...
                 dt .* (Db.*H(t,idx)./(dx^2)  .* ( B(t,idx-1) + B(t,idx+1) - 2.*B(t,idx) ...
    ...
    B(t+1,1)   = B(t,1) + ...
                     dt .* (Db * H(t,1)/(dx^2) .* (B(t,end) + B(t,2) - 2.*B(t,1) ...
                     - (betaP .* (R(t,2) - R(t,1)) ...
                     -  betaBeam .* (R(t,1)- R(t,end)))) ...
                     + gamma .* B(t,1) .* (1 - B(t,1)./Bs));
    betaN   = (Xn(end-1) + Xn(end))./2;
    B(t+1,end) = B(t,end) + ...
                     dt .* (Db * H(t,end)/(dx^2) .* (Db * (B(t,end-1) + B(t,1) - 2.*B(t,end)) ...
                     - (betaBeam .* (R(t,1) - R(t,end)) ...
