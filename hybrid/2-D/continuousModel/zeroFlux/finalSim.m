...
%%set up the movie obj.
vidObj=VideoWriter('finalSim.avi');
  ...
  %% i==J j==1 Top left corner
   ...
   [ Astep(i,j),Bstep(i,j),Rstep(i,j),Hstep(i,j) ] =  ...
    ...
    if (mod(t,250) == 0)
        ...
        subplot(2,2,1);
        surf(X,X,A);shading('flat');xlabel('x');ylabel('y');title('density type A');
        view(0,90);
        hours = dt*t;
        mFigure = gcf;
        ...
        mTextBox.Position = [270 220 80 20];
        mTextBox.BackgroundColor = [1 1 1];
        mTextBox.FontWeight = 'bold';
        subplot(2,2,2);
        surf(X,X,B);shading('flat');xlabel('x');ylabel('y');title('density type B');
        view(0,90);
        zlim([0 1300]);
        subplot(2,2,3);
        surf(X,X,R);shading('flat');xlabel('x');ylabel('y');title('repellent');
        view(0,90);
        subplot(2,2,4);
        surf(X,X,H);shading('flat');xlabel('x');ylabel('y');title('AHL');
        view(0,90);
